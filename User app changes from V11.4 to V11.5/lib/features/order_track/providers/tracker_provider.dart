    ...
    try {
      ...
      if (mapController != null ) {
        ...
        if(_deliveryManModel != null){
          final Branches? branchData = Provider.of<OrderProvider>(Get.context!, listen: false).trackModel?.branches;

          deliverymanLocation = LatLng(_deliveryManModel?.latitude ?? (branchData?.latitude ?? 0), _deliveryManModel?.longitude ?? (branchData?.longitude ?? 0));
        }
